...
export default function QuickActions() {
  return (
    <div className="bg-[#1a1f2e] rounded-lg p-4 sm:p-6 border border-gray-800">
      <h2 className="text-white text-base sm:text-lg font-semibold mb-3 sm:mb-4">Quick Actions</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3 sm:gap-4">
        {actions.map((action) => (
          <button
            key={action.title}
            className="flex items-start gap-3 p-3 sm:p-4 rounded-lg border border-gray-700 hover:border-gray-600 hover:bg-gray-800/50 transition-colors text-left"
          >
            <div className={`p-2 rounded-lg ${action.color} flex-shrink-0`}>
              <action.icon className="w-4 h-4 sm:w-5 sm:h-5 text-white" />
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="text-white font-medium mb-1 text-sm sm:text-base">{action.title}</h3>
